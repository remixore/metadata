...
@app.route('/generate-encrypted-metadata', methods=['POST'])
def generate_encrypted_metadata():
    data = request.form
    user_agent = data.get('user_agent')
    oauth_url = data.get('oauth_url')

    # Perform your logic here to generate the metadata based on user_agent and oauth_url
    # Assuming you have a function named generate_metadata() that returns the metadata

    metadata = meta_audible_app(user_agent, oauth_url)
    encrypted_metadata = encrypt_metadata(metadata)

    return jsonify({'encrypted_metadata': encrypted_metadata})
